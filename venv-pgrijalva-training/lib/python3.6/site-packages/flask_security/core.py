...
_default_messages = {
    ...
    'CONFIRMATION_EXPIRED': (
        _('You did not confirm your email within %(within)s. '
          ...
          'to %(email)s.'), 'error'),
    'LOGIN_EXPIRED': (
        _('You did not auth within %(within)s. New instructions to auth '
          'have been sent to %(email)s.'), 'error'),
    'LOGIN_EMAIL_SENT': (
        _('Instructions to auth have been sent to %(email)s.'), 'success'),
    'INVALID_LOGIN_TOKEN': (
        _('Invalid auth token.'), 'error'),
    'DISABLED_ACCOUNT': (
...
def _get_login_manager(app, anonymous_user):
    ...
    lm.login_view = '%s.auth' % cv('BLUEPRINT_NAME', app=app)
...
def _get_state(app, datastore, anonymous_user=None, **kwargs):
    ...
    kwargs.update(dict(
        ...
        login_serializer=_get_serializer(app, 'auth'),
...
class _SecurityState(object):
    ...
    def login_context_processor(self, fn):
        self._add_ctx_processor('auth', fn)
