...
def send_login_instructions(user):
    """Sends the auth instructions email for the specified user.
...
def generate_login_token(user):
    """Generates a unique auth token for the specified user.
...
def login_token_status(token):
    """Returns the expired status, invalid status, and user of a auth token.
    For example::
    ...
    :param token: The auth token
    """
    return get_token_status(token, 'auth', 'LOGIN')
