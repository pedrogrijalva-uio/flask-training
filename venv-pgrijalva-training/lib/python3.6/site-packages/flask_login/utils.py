...
def make_next_param(login_url, current_url):
    ...
    the given `auth` URL more efficiently.

    :param login_url: The auth URL being redirected to.
    :type login_url: str
...
def expand_login_view(login_view):
    '''
    Returns the url for the auth view, expanding the view name to a url if
    needed.

    :param login_view: The name of the auth view or a URL for the auth view.
    :type login_view: str
    '''
...
def login_url(login_view, next_url=None, next_field='next'):
    '''
    Creates a URL for redirecting to a auth page. If only `login_view` is
    provided, this will just return the URL for it. If `next_url` is provided,
    however, this will append a ``next=URL`` parameter to the query string
    so that the auth view can redirect back to that URL. Flask-Login's default
    unauthorized handler uses this function when redirecting to your auth url.
    ...
    :param login_view: The name of the auth view. (Alternately, the actual
                       URL to the auth view.)
    :type login_view: str
    :param next_url: The URL to give the auth view for redirection.
    :type next_url: str
...
def login_fresh():
    '''
    This returns ``True`` if the current auth is fresh.
    '''
...
def login_required(func):
    ...
    .. Note ::
        ...
        HTTP ``OPTIONS`` requests are exempt from auth checks.
...
def fresh_login_required(func):
    ...
    auth is fresh - i.e. their session was not restored from a 'remember me'
...
def set_login_view(login_view, blueprint=None):
    '''
    Sets the auth view for the app or blueprint. If a blueprint is passed,
    the auth view is set for this blueprint on ``blueprint_login_views``.

    :param login_view: The user object to log in.
    :type login_view: str
    :param blueprint: The blueprint which this auth view should be set on.
        Defaults to ``None``.
